    ...
    checkinForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            if (!data.reflection || !data.suggestion) {
                 throw new Error("Invalid response format from Gemini API. Missing 'reflection' or 'suggestion'.");
            }
...
const questions = [
    { id: 'q1', text: '1. Over the past two weeks, how often have you been bothered by having little interest or pleasure in doing things?' },
    { id: 'q2', text: '2. Over the past two weeks, how often have you been bothered by feeling down, depressed, or hopeless?' }
];
const options = [
...
function renderQuiz() {
    ...
    html += '<button type="submit" class="submit-quiz-btn">Submit Oasis Check-in</button>';
...
if (resourceButton) {
    resourceButton.addEventListener('click', () => {
        console.log("Redirecting to a page with crisis hotlines, therapy links, and self-care articles.");
        alert(
